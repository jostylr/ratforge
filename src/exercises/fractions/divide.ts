...
export interface DivideFractionsParams {
  num1: number;
  den1: number;
  num2: number;
  den2: number;
...
export const divideFractionsExercise: Exercise = {
  id: "frac-divide",
  topic: "fractions",
  title: "Divide Fractions",
  description: "Divide fractions using Keep-Change-Flip",
  difficulty: 3,
  ...
  generate(seed?: number): ExerciseInstance {
    ...
    const num1 = randomInt(1, 4, random);
    const den1 = randomInt(num1 + 1, 6, random);
    const num2 = randomInt(1, 3, random);
    const den2 = randomInt(num2 + 1, 5, random);
    
    // Keep-Change-Flip: (num1/den1) ÷ (num2/den2) = (num1/den1) × (den2/num2)
    let answerNum = num1 * den2;
    let answerDen = den1 * num2;
  ...
  validate(instance: ExerciseInstance, answer: unknown): ValidationResult {
    const params = instance.params as unknown as DivideFractionsParams;
    ...
    if (parts.length === 2) {
      ...
      if (num === params.answerNum && den === params.answerDen) {
        return { correct: true, feedback: `Correct! Keep-Change-Flip gives ${params.answerNum}/${params.answerDen} 🎉` };
      }
    }
    
    return { correct: false, feedback: `Not quite. Remember: Keep the first fraction, Change ÷ to ×, Flip the second fraction.` };
  },
  ...
  renderHTML(instance: ExerciseInstance): string {
    ...
    return `
      <div class="exercise-container" x-data="divideFractionsExercise()" x-init="$nextTick(() => $refs.numInput?.focus())">
        <div class="exercise-prompt">
          <h2>Divide Fractions</h2>
          <p class="exercise-hint">Keep-Change-Flip: Keep first, change ÷ to ×, flip second</p>
        </div>
        ...
        <div class="feedback-area" x-show="feedback" x-cloak>
          ...
          <div class="next-actions" x-show="submitted">
            ...
            <a href="/practice/frac-divide" class="btn btn-primary" x-show="correct || givenUp" x-ref="nextBtn">
              Next Exercise
            </a>
      ...
      <script>
        function divideFractionsExercise() {
          return {
            answerNum: '',
            answerDen: '',
            activeField: 'num',
