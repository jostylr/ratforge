...
export interface SubtractDiffDenomParams {
  num1: number;
  den1: number;
  num2: number;
  den2: number;
...
export const subtractDiffDenomExercise: Exercise = {
  id: "frac-sub-diff",
  topic: "fractions",
  title: "Subtract Fractions (Different Denominators)",
  ...
  generate(seed?: number): ExerciseInstance {
    ...
    // Use denominators with manageable LCMs
    const denPairs: [number, number][] = [[2, 3], [2, 4], [3, 4], [2, 6], [3, 6], [4, 6], [2, 5], [3, 5]];
    const pair = denPairs[randomInt(0, denPairs.length - 1, random)]!;
    const den1 = pair[0];
    const den2 = pair[1];
    
    const lcd = lcm(den1, den2);
    
    // Make sure first fraction is larger
    const num1 = randomInt(Math.ceil(den1 / 2), den1 - 1, random);
    const num2 = randomInt(1, Math.floor(den2 / 2), random);
    
    // Convert to LCD and subtract
    const converted1 = num1 * (lcd / den1);
    const converted2 = num2 * (lcd / den2);
    
    let answerNum = converted1 - converted2;
    let answerDen = lcd;
    ...
    return {
      ...
      exerciseId: this.id,
      seed: actualSeed,
      params: { num1, den1, num2, den2, answerNum, answerDen, lcd } as unknown as Record<string, unknown>,
  ...
  validate(instance: ExerciseInstance, answer: unknown): ValidationResult {
    const params = instance.params as unknown as SubtractDiffDenomParams;
    ...
    return { correct: false, feedback: `Not quite. Find a common denominator first (LCD = ${params.lcd}).` };
  },
  ...
  renderHTML(instance: ExerciseInstance): string {
    ...
    return `
      <div class="exercise-container" x-data="subtractDiffDenomExercise()" x-init="$nextTick(() => $refs.numInput?.focus())">
        <div class="exercise-prompt">
          <h2>Subtract Fractions</h2>
          <p class="exercise-hint">Find a common denominator, then subtract</p>
        </div>
        ...
        <div class="feedback-area" x-show="feedback" x-cloak>
          ...
          <div class="next-actions" x-show="submitted">
            ...
            <a href="/practice/frac-sub-diff" class="btn btn-primary" x-show="correct || givenUp" x-ref="nextBtn">
              Next Exercise
            </a>
      ...
      <style>
        ...
        .frac-input:focus { outline: none; border-style: solid; }
        .numpad-section {
          display: flex;
          flex-direction: column;
          align-items: center;
      ...
      <script>
        function subtractDiffDenomExercise() {
          return {
            answerNum: '',
            answerDen: '',
            activeField: 'num',
