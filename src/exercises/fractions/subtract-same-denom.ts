...
export interface SubtractFractionsParams {
  num1: number;
  num2: number;
  denominator: number;
  diffNum: number;
...
export const subtractFractionsExercise: Exercise = {
  id: "frac-sub-same",
  topic: "fractions",
  ...
  generate(seed?: number): ExerciseInstance {
    ...
    const num1 = randomInt(3, denominator, random);
    ...
    return {
      ...
      exerciseId: this.id,
      seed: actualSeed,
      params: { num1, num2, denominator, diffNum } as unknown as Record<string, unknown>,
      correctAnswer: diffNum,
      createdAt: new Date().toISOString(),
    };
  },

  validate(instance: ExerciseInstance, answer: unknown): ValidationResult {
    const params = instance.params as unknown as SubtractFractionsParams;
    const answerNum = typeof answer === "number" ? answer : parseInt(String(answer), 10);
    
    if (answerNum === params.diffNum) {
      return { correct: true, feedback: `Correct! ${params.num1}/${params.denominator} - ${params.num2}/${params.denominator} = ${params.diffNum}/${params.denominator} 🎉` };
    }
    
    return { correct: false, feedback: `Not quite. When denominators are the same, just subtract the numerators!` };
  },
  ...
  renderHTML(instance: ExerciseInstance): string {
    ...
    return `
      <div class="exercise-container" x-data="subtractFractionsExercise()" x-init="$nextTick(() => document.querySelector('.answer-input')?.focus())">
        <div class="exercise-prompt">
          <h2>Subtract the Fractions</h2>
          <p class="exercise-hint">When denominators are the same, subtract the numerators.</p>
        </div>
        ...
        <div class="feedback-area" x-show="feedback" x-cloak>
          ...
          <div class="next-actions" x-show="submitted">
            ...
            <a href="/practice/frac-sub-same" class="btn btn-primary" x-show="correct || givenUp" x-ref="nextBtn">
              Next Exercise
            </a>
      ...
      <script>
        function subtractFractionsExercise() {
          return {
            answer: '',
            showNumpad: true,
            submitted: false,
