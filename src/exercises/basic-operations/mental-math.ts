...
export interface MentalMathParams {
  expression: string;
  answer: number;
  strategy: string;
}

export const mentalMathExercise: Exercise = {
  id: "basic-mental",
  ...
  generate(seed?: number): ExerciseInstance {
    ...
    const type = randomInt(0, 4, random);
    let expression: string, answer: number, strategy: string;
    
    if (type === 0) {
      // Add 9 (add 10, subtract 1)
      const a = randomInt(10, 50, random);
      expression = `${a} + 9`;
      answer = a + 9;
      strategy = "Add 10, subtract 1";
    } else if (type === 1) {
      // Add 99 (add 100, subtract 1)
      const a = randomInt(50, 200, random);
      expression = `${a} + 99`;
      answer = a + 99;
      strategy = "Add 100, subtract 1";
    } else if (type === 2) {
      // Double and halve
      const a = randomInt(2, 12, random) * 2;
      const b = randomInt(2, 6, random) * 5;
      expression = `${a} × ${b}`;
      answer = a * b;
      strategy = "Double one, halve the other";
    } else if (type === 3) {
      // Subtract from round number
      const round = [100, 200, 500][randomInt(0, 2, random)]!;
      const b = randomInt(10, round / 2, random);
      expression = `${round} - ${b}`;
      answer = round - b;
      strategy = "Count up from the smaller number";
    } else {
      // Add by making 10s
      const a = randomInt(10, 40, random);
      const complement = 10 - (a % 10);
      const b = complement + randomInt(0, 3, random) * 10;
      expression = `${a} + ${b}`;
      answer = a + b;
      strategy = "Make a 10 first";
    }
    ...
    return {
      ...
      exerciseId: this.id,
      seed: actualSeed,
      params: { expression, answer, strategy } as unknown as Record<string, unknown>,
  ...
  validate(instance: ExerciseInstance, answer: unknown): ValidationResult {
    const params = instance.params as unknown as MentalMathParams;
    ...
    return { correct: false, feedback: `Not quite. Try: ${params.strategy}` };
  },
  ...
  renderHTML(instance: ExerciseInstance): string {
    ...
    return `
      <div class="exercise-container" x-data="mentalMathExercise()" x-init="$nextTick(() => $refs.mainInput?.focus())">
        <div class="exercise-prompt">
          <h2>Mental Math</h2>
          <p class="exercise-hint">Solve quickly in your head!</p>
        </div>
        ...
        <div class="feedback-area" x-show="feedback" x-cloak>
          ...
          <div class="next-actions" x-show="submitted">
            ...
            <a href="/practice/basic-mental" class="btn btn-primary" x-show="correct || givenUp" x-ref="nextBtn">
              Next Exercise
            </a>
